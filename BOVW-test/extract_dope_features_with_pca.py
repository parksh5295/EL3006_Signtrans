...
def forward_features(self, images, targets=None):
    # type: (List[Tensor], Optional[List[Dict[str, Tensor]]]) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]]]
        ...
        detections, detector_losses, dope_features = self.roi_heads(features, proposals, images.image_sizes, targets)
        #return features, dope_features
        return dope_features.detach().cpu().numpy()
...
def process_video(filename, ckpt):
    ...
    results = np.zeros((video_length, 62, 2048))
    results = []
    ...
    while cap.isOpened():
        ...
        with torch.no_grad():
            ...
            results.append(forward_features(model, imlist))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pca_model_path', type=str, help='path of saved pca_model', required=True)
    parser.add_argument('--vocab', type=str, help='path to vocabulary', required=True)
    parser.add_argument('--idf', type=str, help='path to idf', required=True)
    ...
    with open(args.pca_model_path, "rb") as f:
        pca = pickle.load(f)

    feature_vocab = np.load(args.vocab)
    ...
    for video_name in tqdm(os.listdir(args.video_dir)):
        ...
        out_reduced = []
        for o in out:
            out_reduced.append(pca.transform(o))
       # np.save(os.path.join(args.out_dir, video_name[:-4]), out_reduced)

        arrays = out_reduced
        dst_filename = os.path.join(args.out_dir, video_name[:-4])

        descriptors = np.zeros((len(arrays), num_words))
        for i, array in enumerate(arrays):
            _, clusters = tree.query(array, k=1)
            query_hist, _ = np.histogram(clusters.ravel(), bins = num_words)
            query_hist = query_hist * idf
            query_hist = np.sqrt(query_hist)
            query_hist = query_hist / np.linalg.norm(query_hist)
            descriptors[i, :] = query_hist
        np.save(dst_filename, descriptors)
