...
from signjoey.model import build_model, SignModel, SignModelEnsemble, build_ensemble_model
...
def test(
    cfg_file, ckpts: [], output_path: str = None, logger: logging.Logger = None
) -> None:
    """
    ...
    if ckpts is None:
        ckpts = []
        models_dirs = cfg["training"]["model_dir"]
        for idx, model_dir in enumerate(model_dirs):
            ckpts.append(get_latest_checkpoint(model_dir))
            if ckpts[idx] is None:
    ...
    model_checkpoints = [load_checkpoint(ckpt, use_cuda=use_cuda) for ckpt in ckpts]

    # build model and load parameters into it
    do_recognition = cfg["training"].get("recognition_loss_weight", 1.0) > 0.0
    do_translation = cfg["training"].get("translation_loss_weight", 1.0) > 0.0
    models = []
    cfg_data_feature_sizes = []
    for idx, model_checkpoint in enumerate(model_checkpoints):   
        cfg_data_feature_size = cfg["data"]["feature_size"][idx]
        cfg_data_feature_sizes.append(cfg_data_feature_size)
        model = build_model(
                cfg=cfg["model"],
                gls_vocab=gls_vocab,
                txt_vocab=txt_vocab,
                sgn_dim=sum(cfg_data_feature_size)
                if isinstance(cfg_data_feature_size, list)
                else cfg_data_feature_size,
    ...
    if do_recognition:
        ...
        for rbw in recognition_beam_sizes:
            ...
            dev_recognition_results[rbw] = validate_on_data(
                model=ensemble_model, # NOTE CHANGED
                data=dev_data,
                batch_size=batch_size,
                use_cuda=use_cuda,
                batch_type=batch_type,
                dataset_version=dataset_version,
                sgn_dims=cfg_data_feature_sizes,
