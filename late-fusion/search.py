...
def transformer_greedy(
    ...
    Ys = [output.new_full([batch_size, 1], bos_index, dtype=torch.long) for output in encoder_output]
    ...
    for _ in range(max_output_length):
        ...
        trg_embeds = []
        for ys in Ys:
          trg_embeds.append(embed(ys))  # embed the previous tokens
        ...
        with torch.no_grad():
            ...
            for i in range(len(decoders)):
                ...
                logits, out, _, _ = decoders[i](
                    trg_embed=trg_embeds[i],
            ...
            logits = logits[0] * 0.7 + logits[1] * 0.15 + logits[2] * 0.15
            #logits = torch.mean(torch.stack(logits_list), 0)
            _, next_word = torch.max(logits, dim=1)
            next_word = next_word.data
            for i, ys in enumerate(Ys):
              Ys[i] = torch.cat([ys, next_word.unsqueeze(-1)], dim=1)
...
def beam_search_ensemble(
    decoders, # NOTE Changed
    size: int,
    bos_index: int,
    eos_index: int,
    pad_index: int,
    encoder_outputs,  # NOTE Changed
    encoder_hiddens,  # NOTE Changed
    src_mask: Tensor,
    max_output_length: int,
    alpha: float,
    embed: Embeddings,
    ...
    # init
    transformer = isinstance(decoders[0], TransformerDecoder) # NOTE CHANGED
    batch_size = src_mask.size(0)
    att_vectors = None  # not used for Transformer

    hidden = None # NOTE CHANGED

    for i in range(len(encoder_outputs)):
        encoder_outputs[i] = tile(
                encoder_outputs[i].contiguous(), size, dim=0
    ...
    batch_offset = torch.arange(
        batch_size, dtype=torch.long, device=encoder_outputs[0].device
    )

    # numbering elements in the extended batch, i.e. beam size copies of each
    # batch element
    beam_offset = torch.arange(
            0, batch_size * size, step=size, dtype=torch.long, device=encoder_outputs[0].device
            )
    ...
    topk_log_probs = torch.zeros(batch_size, size, device=encoder_outputs[0].device)
    ...
    for step in range(max_output_length):
        ...
        # NOTE CHANGED
        # NOTE AVERAGE OVER LOG_PROBS
        log_probs = torch.stack(log_probs_list)
        log_probs = log_probs[0] * 0.7 + log_probs[1] * 0.15 + log_probs[2] * 0.15
        #log_probs = torch.mean(torch.stack(log_probs_list), 0)
        ...
        curr_scores = curr_scores.reshape(-1, size * decoders[0].output_size)
        ...
        topk_beam_index = topk_ids.div(decoders[0].output_size)
        topk_ids = topk_ids.fmod(decoders[0].output_size)
        ...
        for i in range(len(encoder_outputs)):
            encoder_outputs[i] = encoder_outputs[i].index_select(0, select_indices)
