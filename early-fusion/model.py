...
class SignModel(nn.Module):
    ...
    def encode(
        ...
        embed_src = []
        start = 0
        for i in range(len(self.sgn_dims)):
            end = start + self.sgn_dims[i]
            embed_src.append(self.sgn_embeds[i](x=sgn[:, :, start:end], mask=sgn_mask))
            start = end
        embed_src = torch.cat(tuple(embed_src), 2).cuda()
        return self.encoder(
            embed_src=embed_src,
...
def build_model(
    ...
    sgn_embeds = []
    for sgn_dim, emb in zip(sgn_dims, cfg["embeddings"]):
        sgn_embeds.append(SpatialEmbeddings(
                **emb,
